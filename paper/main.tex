...
\subsection{INT8 Quantization Setup}
Models are exported to ONNX and quantized with ONNX Runtime dynamic per-tensor INT8 (CPUExecutionProvider, no calibration dataset; weights quantized to INT8, activations dynamically quantized at runtime). This matches the stock ORT PTQ path and explains the architecture-dependent accuracy drop (including EfficientNet-B0 SE blocks).

\subsection{x86 Latency Protocol}
x86 latency is measured on an Intel i5-12400F CPU (CPUExecutionProvider) with 200 runs and 10 warmup iterations per model; we report mean, P50, P95, P99, throughput (FPS), and model size. Power is not measured on x86.
...
\begin{table}[htbp]
\caption{FP32 test accuracy (publisher test split, \texttt{experiment\_results.json}); INT8 in Table~\ref{tab:quantacc} (\texttt{eval\_int8.py} differs by 0.63\,pp MN2, 0.17\,pp others). Precision/recall micro-averaged.}
\label{tab:results}
\centering
\begin{tabular}{lccc}
\toprule
...
EfficientNet-B0 achieves the highest accuracy (98.35\%), consistent with its compound scaling design that balances feature hierarchy depth. This result is close to Dhungana~\textit{et al.}'s 99.12\%~\cite{dhungana2025} on the same dataset, while our approach provides additional edge deployment analysis. Per-class test accuracy (FP32) is strongest on Salmonellosis (99.29\% for EfficientNet-B0) and weakest on the underrepresented NCD class (90.97--93.75\% across models); full per-class FP32/INT8 numbers are in Table~\ref{tab:quantacc}. Misclassifications concentrate on NCD: for MobileNetV2, 8 of 144 NCD images spread across all three other classes (3 as cocci, 1 as healthy, 4 as salmo), while the largest off-diagonal confusion is 11 healthy images predicted as cocci; a similar NCD spread is visible for ShuffleNetV2 and EfficientNet-B0 (\texttt{figures/cm\_*.png}). This mirrors the healthy$\leftrightarrow$NCD confusion reported by Tasdelen and Arslan~\cite{tasdelen2025} and indicates visual overlap rather than a single confusable pair.
...
\begin{table}[htbp]
\caption{FP32 vs INT8 accuracy (publisher test split, \texttt{eval\_int8.py}); per-class cocci / healthy / ncd / salmo. MN2 and SNV2 robust; EB0 collapses (salmo 0.53\%).}
\label{tab:quantacc}
\centering
\footnotesize
...
\begin{table}[htbp]
\caption{RPi5 latency, batch=1 (mean of 200 runs, P50 in parentheses). 44--58$^{\circ}$C, throttled=0x0; std $<$1.6\,ms except 4-thread tails.}
\label{tab:rpi5}
\centering
\footnotesize
...
\begin{table}[htbp]
\caption{Level-2: node expansion and ORT quant-kernel overhead (RPi5, 10 runs, batch=1, 1 thread). Dynamic PTQ expands Conv into DynamicQuantizeLinear + ConvInteger + Mul + Cast (no static Q/DQ nodes).}
\label{tab:level2}
\centering
\footnotesize
\setlength{\tabcolsep}{3.0pt}
...
\caption{Accuracy--latency--size trade-off (circles FP32; crosses measured INT8). MN2-FP32 lowest x86 latency (1.85\,ms, 97.83\%); EB0-FP32 highest accuracy (98.35\%, 3.50\,ms). EB0-INT8 off-scale annotated; SNV2-FP32 dominated by MN2-FP32 (no formal Pareto claimed).}
\label{fig:pareto}
\end{figure}
...
\caption{Grad-CAM for MobileNetV2 (qualitative, one example per class). Heatmaps localize the fecal region; no quantitative attention metric claimed.}
\label{fig:gradcam}
\end{figure}
...
Three limitations bound these claims. First, results come from a single seed (42) with best-checkpoint selection on validation accuracy; multi-seed variance is unreported. Second, energy is unmeasured: current/power telemetry was unavailable on our RPi5 board (\texttt{current\_a=null}), and x86 power was not recorded, so deployment cost is discussed in latency, memory, and size only. Third, the NCD class is underrepresented (5.5\% of Zenodo; 8.3\% raw) and Grad-CAM evidence is qualitative single-example; a quantitative attention-overlap metric and class-imbalance mitigation remain future work alongside QAT comparison.

\section{Conclusion}

This paper characterizes lightweight CNN architectures with INT8 quantization for poultry fecal disease detection. Our key findings are: (1) all three architectures exceed 97\% accuracy, so hardware constraints should drive model choice; (2) INT8 dynamic PTQ increases latency on CPU-only stacks (6.4--30.1$\times$ on x86, 1.9--2.8$\times$ on RPi5) with flat ARM thread scaling; (3) INT8 accuracy is architecture-dependent, with EfficientNet-B0 collapsing to 35.46\% while MobileNetV2 and ShuffleNetV2 stay robust. The trade-off identifies MobileNetV2 FP32 for latency-critical use and ShuffleNetV2 INT8 for size-constrained use.

These findings enable practitioners to make informed deployment decisions based on their specific hardware constraints. Future work includes multi-seed variance reporting, QAT comparison, energy measurement on RPi5, and class imbalance mitigation for the underrepresented NCD class.

\section*{Data Availability}
The publisher test split, logs, benchmark JSONs, split manifest, and Level-2 artifact are in \texttt{results/}; figures in \texttt{figures/} and \texttt{paper/}. Source dataset: HuggingFace \texttt{Dianyo/poultry-fecal-fl}~\cite{chi2026}.
