...
\begin{itemize}
    \item First systematic comparison of three lightweight CNNs with INT8 quantization characterization for poultry fecal disease detection, validated on both x86 and ARM (Raspberry Pi~5).
    \item Empirical demonstration that INT8 PTQ on CPU introduces 6--30$\times$ latency overhead on x86 and 1.9--2.8$\times$ on ARM, challenging the assumption that quantization universally improves inference speed; INT8 thread scaling on ARM stalls at 1.1--1.2$\times$ versus 2.3$\times$ for FP32.
    \item Hardware-aware deployment guidelines grounded in cross-platform characterization (latency, thread scaling, batch scaling, memory) and a Pareto/interpretability analysis.
\end{itemize}
...
Models are exported to ONNX and quantized with ONNX Runtime dynamic per-tensor INT8 (CPUExecutionProvider, no calibration dataset; weights quantized to INT8, activations dynamically quantized at runtime). This matches the stock ORT PTQ path and explains the architecture-dependent accuracy drop (notably EfficientNet-B0 SE sensitivity). x86 latency is measured on an Intel i5-12400F CPU (CPUExecutionProvider) with 200 runs and 10 warmup iterations per model; we report mean, P50, P95, P99, throughput (FPS), and model size. Power is not measured on x86.
...
Table~\ref{tab:rpi5} and Fig.~\ref{fig:rpi5thread} summarize RPi5 latency at batch=1, the realistic streaming mode for farm deployment. FP32 scales with threads: MobileNetV2 drops from 33.13\,ms (1 thread, 30.2\,FPS) to 14.35\,ms (4 threads, 69.7\,FPS, 2.31$\times$ speedup, 57.7\% parallel efficiency); ShuffleNetV2 from 14.20\,ms to 6.26\,ms (2.27$\times$, 159.7\,FPS); EfficientNet-B0 from 70.06\,ms to 31.20\,ms (2.25$\times$). The sweet spot is 3 threads (e.g., MobileNetV2 14.99\,ms at 3 threads vs.\ 14.35\,ms at 4 threads with higher variance, std 1.53\,ms), indicating diminishing returns beyond 3 threads for FP32.
...
\textbf{Why INT8 is slower: evidence, not buzzwords.} On CPUExecutionProvider without VNNI (x86) or DOTPROD/NEON INT8 matmul acceleration, ORT inserts QuantizeLinear/DequantizeLinear nodes and executes the heavy matmuls in FP32 after dequantizing activations. Thus INT8 adds work (quant/dequant + shape dispatch) without reducing the matmul cost. The cost is proportional to the number of quantize boundaries: depthwise separable + SE blocks (MobileNetV2, EfficientNet-B0) create many small matmuls and are hit hardest (30.1$\times$ on x86, 2.76$\times$ on ARM), while homogeneous channel-shuffle blocks (ShuffleNetV2) are most resilient (6.4$\times$ / 1.89$\times$). The flat thread scaling further supports a kernel-level bottleneck: dequantize is memory-bound and poorly parallelized, so adding threads does not help, unlike FP32 GEMM which is compute-bound and scales.
...
\begin{enumerate}
    ...
    \item INT8 PTQ on CPU introduces 6.4--30.1$\times$ latency overhead on x86 and 1.9--2.8$\times$ on RPi5, challenging the assumption that quantization universally improves inference speed; INT8 thread scaling stalls at 1.1--1.2$\times$ on ARM versus 2.3$\times$ for FP32.
    \item ShuffleNetV2 exhibits the most resilient quantization behavior (6.4$\times$ on x86, 1.89$\times$ on RPi5) due to architectural homogeneity.
    \item On RPi5, batch=1 streaming is optimal (per-image latency and RSS both increase with batch), and INT8 file-size reduction (3.3--3.7$\times$) does not translate to lower runtime RSS.
    \item The Pareto frontier identifies MobileNetV2 as optimal for latency-critical deployment (1.85\,ms, 541\,FPS on x86; 6.26\,ms, 159.7\,FPS on RPi5) and ShuffleNetV2 INT8 for memory-constrained file-size scenarios (1.47\,MB).
\end{enumerate}
